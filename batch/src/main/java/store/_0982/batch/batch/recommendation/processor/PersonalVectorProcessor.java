...
public class PersonalVectorProcessor implements ItemProcessor<List<MemberVectorInput>, List<PersonalVector>> {

    @Override
    public List<PersonalVector> process(List<MemberVectorInput> item) {
        if (item == null || item.isEmpty()) {
            return List.of();
        }
        List<PersonalVector> results = new ArrayList<>(item.size());
        for (MemberVectorInput input : item) {
