...
import store._0982.notification.exception.notification.CustomKafkaException;
...
public class GroupPurchaseEventListener {
    ...
    public void handleGroupPurchaseChangedEvent(GroupPurchaseChangedEvent event) {
    ...
    private NotificationContent createContent(GroupPurchaseChangedEvent event) {
        return switch (event.getStatus()) {
            case SUCCESS -> new NotificationContent(
                    NotificationType.GROUP_PURCHASE_COMPLETED,
                    "공동 구매 성사 완료",
                    String.format("공동 구매가 성사되어 %,d원이 정산금에 추가될 예정입니다.", event.getTotalAmount()));
