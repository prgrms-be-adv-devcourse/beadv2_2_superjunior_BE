...
class PaymentFailServiceTest {
    ...
    void handlePaymentFailure_success() {
        // given
        Payment payment = Payment.create(memberId, orderId, 10000L);

        PaymentFailCommand command = new PaymentFailCommand(
                orderId,
                "test_payment_key",
                "PAYMENT_FAILED",
                "카드 승인 실패",
    ...
    void handlePaymentFailure_exception_when_completed() {
        ...
        Payment payment = Payment.create(memberId, orderId, 10000L);
        payment.markConfirmed("간편 결제", OffsetDateTime.now(), paymentKey);
