...
class GroupPurchaseControllerTest {
    ...
    @Nested
    @DisplayName("공동구매 판매자별 목록 조회 API")
    class GetGroupPurchasesBySellerTest {

        @Test
        @DisplayName("판매자별 공동구매 목록을 페이징하여 조회합니다.")
        void getGroupPurchasesBySeller_success() throws Exception {
            // given
            UUID sellerId = UUID.randomUUID();
            OffsetDateTime now = OffsetDateTime.now();
            ...
            when(groupPurchaseService.getGroupPurchasesBySeller(any(), any()))
                    .thenReturn(pageResponse);

            // when & then
            mockMvc.perform(
                            get("/api/purchases/seller/{sellerId}", sellerId)
                                    .param("page", "0")
                                    .param("size", "10")
                    )
                    .andExpect(status().isOk())
                    ...
                    .andExpect(jsonPath("$.data.content[0].title").value("판매자 공동구매 1"))
                    .andExpect(jsonPath("$.data.content[0].minQuantity").value(10))
                    .andExpect(jsonPath("$.data.content[0].maxQuantity").value(100))
                    .andExpect(jsonPath("$.data.content[0].discountedPrice").value(15000))
                    .andExpect(jsonPath("$.data.content[0].currentQuantity").value(50))
                    ...
                    .andExpect(jsonPath("$.data.content[1].title").value("판매자 공동구매 2"))
                    .andExpect(jsonPath("$.data.content[1].minQuantity").value(5))
                    .andExpect(jsonPath("$.data.content[1].maxQuantity").value(50))
                    .andExpect(jsonPath("$.data.content[1].discountedPrice").value(25000))
                    .andExpect(jsonPath("$.data.content[1].currentQuantity").value(30))
            ...
            verify(groupPurchaseService, times(1)).getGroupPurchasesBySeller(any(), any());
        }

        @Test
        @DisplayName("해당 판매자의 공동구매가 없어도 정상 조회됩니다.")
        void getGroupPurchasesBySeller_empty() throws Exception {
            // given
            UUID sellerId = UUID.randomUUID();
            PageResponse<GroupPurchaseThumbnailInfo> pageResponse = new PageResponse<>(
                    List.of(),
        ...
        @Test
        @DisplayName("잘못된 sellerId UUID 형식이면 400 에러가 발생합니다.")
        void getGroupPurchasesBySeller_invalidUUID() throws Exception {
            // when & then
            mockMvc.perform(
                            get("/api/purchases/seller/{sellerId}", "uuid")
                                    .param("page", "0")
                                    .param("size", "10")
                    )
                    .andExpect(status().isBadRequest())
