...
public class OrderKafkaConsumerConfig {
    ...
    @Bean
    public ConsumerFactory<String, PointEvent> orderConsumerFactory() {
        return KafkaCommonConfigs.defaultConsumerFactory(bootstrapServers, "order-service-group");
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, PointEvent> orderKafkaListenerFactory() {
