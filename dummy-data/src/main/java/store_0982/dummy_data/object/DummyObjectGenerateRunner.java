package store_0982.dummy_data.object;
...
import store_0982.dummy_data.object.commerce.DummyGroupPurchaseGenerator;
import store_0982.dummy_data.object.commerce.DummyOrderGenerator;
import store_0982.dummy_data.object.commerce.DummyCanceledOrderGenerator;
import store_0982.dummy_data.object.commerce.DummyOrderSettlementGenerator;
import store_0982.dummy_data.object.commerce.DummyProductGenerator;
import store_0982.dummy_data.object.commerce.DummySellerBalanceGenerator;
import store_0982.dummy_data.object.member.DummyMemberGenerator;
import store_0982.dummy_data.object.member.DummySellerGenerator;
import store_0982.dummy_data.object.point.DummyPointBalanceGenerator;
import store_0982.dummy_data.object.recommendation.DummyProductVectorGenerator;
...
public class DummyObjectGenerateRunner implements ApplicationRunner {
    ...
    private final DummyPointBalanceGenerator dummyPointBalanceGenerator;
    private final DummySellerBalanceGenerator dummySellerBalanceGenerator;
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (targets.contains(GeneratorType.POINT_BALANCE)) {
            log.info("[더미데이터 포인트 잔액 데이터 생성 시작.]");
            dummyPointBalanceGenerator.generate();
            log.info("[더미데이터 포인트 잔액 데이터 생성 완료.]");
        }

        if (targets.contains(GeneratorType.SELLER_BALANCE)) {
            log.info("[더미데이터 판매자 정산 잔액 데이터 생성 시작.]");
            dummySellerBalanceGenerator.generate();
            log.info("[더미데이터 판매자 정산 잔액 데이터 생성 완료.]");
        }
    ...
    private java.util.EnumSet<GeneratorType> parseTargets(ApplicationArguments args) {
        ...
        for (String token : raw.split(",")) {
            ...
            switch (key) {
                ...
                case "point-balance", "point_balance", "pointbalance" -> set.add(GeneratorType.POINT_BALANCE);
                case "seller-balance", "seller_balance", "sellerbalance" -> set.add(GeneratorType.SELLER_BALANCE);
                case "order" -> set.add(GeneratorType.ORDER);
                case "canceled-order", "canceled_order", "canceledorder" -> set.add(GeneratorType.CANCELED_ORDER);
                case "order-settlement", "order_settlement", "ordersettlement" -> set.add(GeneratorType.ORDER_SETTLEMENT);
